...
def add_ai_input(self, context):
    layout = self.layout
    layout.prop(context.scene, "beebox_ai_prompt")
    layout.prop(context.scene, "beebox_ai_reset")


def on_input_enter(self, context):
    if context.scene.beebox_ai_prompt != "":
        bpy.ops.beebox_ai.send_message()
        context.scene.beebox_ai_prompt = ""


def register():
    install_packages(REQUIRED_PACKAGES)
    bpy.types.Scene.beebox_ai_prompt = bpy.props.StringProperty(
        name="AI Prompt",
        description="Input for AI text",
        default="",
        update=on_input_enter,
    )
    bpy.types.Scene.beebox_ai_reset = bpy.props.BoolProperty(
        name="Reset Code",
        description="Delete existing code in the file before creating a new one",
        default=False,
    )
...
def unregister():
    ...
    del bpy.types.Scene.beebox_ai_prompt
    del bpy.types.Scene.beebox_ai_reset
